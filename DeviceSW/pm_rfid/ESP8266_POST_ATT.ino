...
const char* server_valid_card = "http://permitmemass.ko-aaham.com/vRFID";
const char* server_device_data = "http://permitmemass.ko-aaham.com/sDevData";

//const char* server_valid_card = "http://192.168.49.165/vRFID";
//const char* server_device_data = "http://192.168.49.165/sDevData";
...
up:
  ...
  if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
    HTTPClient http;
    // Your Domain name with URL path or IP address with path
    http.begin(server_valid_card);
    // If you need an HTTP request with a content type: application/json, use the following:
    http.addHeader("Content-Type", "application/json");
#ifdef _DEBUG
    Serial.println("{\"deviceid\":\"" + deviceid + "\",\"cardid\":\"" + content + "\"}");
#endif
    int httpResponseCode = http.POST("{\"deviceid\":\"" + deviceid + "\",\"cardid\":\"" + content + "\"}");
    ...
    userid = getValue(payload, '"', 7);
#ifdef _DEBUG
    Serial.print("userid = ");
    Serial.println(userid);
#endif

    username = getValue(payload, '"', 11);
#ifdef _DEBUG
    Serial.print("username = ");
    Serial.println(username);
#endif

    flagst = getValue(payload, '"', 15);
#ifdef _DEBUG
    Serial.print("flag = ");
    ...
    if (payload.startsWith ("<!DOCTYPE html>", 5)) {
      Serial.println("Got an error from the server");
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Invalid Card pls");
      lcd.setCursor(0, 1);
      lcd.print("use a valid card");
      err_rled_buz_3();
      delay(2000);
    }
    ...
    else if (sizeof(username) > 6 && flagst == "0") {
      valid_user = false;
      lcd.clear();
      lcd.setCursor(0, 0);
...
tps:
    ...
    if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
      ...
      //#ifdef _DEBUG
      //      Serial.println("{\"deviceid\":\"" + deviceid + "\",\"userid\":\"" + userid + "\",\"temp\":\"" + tempavg + "\",\"spo2\":\"" + spo2 + "\"}");
      //#endif
