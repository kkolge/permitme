...
<?php $counter = 1; ?>
    ...
    <p class="h1">Pincode wise 15 days historical data for {{ $city }}</p>
    ...
    <br/>
        ...
        @foreach($repCollect as $c)
            <tr class="text-light">
                <td>{{$counter++}} </td>
                <td><a href="/adminReports/sPincodeReport?source={{$state.'.'.$district.'.'.$taluka.'.'.$city.'.'.$c['pincode']}}&type=HighTemp"> <div class="btn-link">{{ $c['pincode'] }}</div> </a> </td>
                <td>{{$c['allAbnormal']}}</td>
                <td>{{$c['hbCount']}}</td>
                <td>{{$c['spo2Count']}}</td>
                <td>{{$c['tempCount']}}</td>
    ...
    <p>
        <div class="flex">
        <div class="mx-auto">
                <a href="/adminReports/sCityReport?source={{$source}}&type=download" class="btn btn-info">Download</a>
            </div>
            <div class="mx-auto">
