...
<?php $counter = 1; ?>
    ...
    <p class="h1">District wise 15 days historical data for {{ $state }} </p>
    ...
    <br/>
        ...
        @foreach($repCollect as $c)
            <tr class="text-light">
                <td>{{$counter++}} </td>
                <td><a href="/adminReports/sDistrictReport?source={{$state.".".$c['district']}}&type=All"> <div class="btn-link">{{ $c['district'] }}</div> </a></td>
                <td>{{$c['allAbnormal']}}</td>
                <td>{{$c['hbCount']}}</td>
                <td>{{$c['spo2Count']}}</td>
                <td>{{$c['tempCount']}} </td>
